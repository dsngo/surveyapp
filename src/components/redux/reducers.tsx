import { SET_SEARCH_TERM, ADD_NEW_QUESTION, UPDATE_QUESTION, REMOVE_QUESTION, UPDATE_INFO_SURVEY, 
  UPDATE_QUESTION_TYPE, UPDATE_TEMP_SURVEY_ID, GET_DATA_FROM_DB_BY_ID, CLEAR_SURVEY,
  UPDATE_SELECTED_QUESTION_TYPE } from "./actions";
import {
  // ICheckBox,
  // IDropdown,
  // ILongQuestion,
...
const surveyContents = (state: any[], action: any) =>
  ...
  (action.type === ADD_NEW_QUESTION && (state.splice((action.currentIndex || action.currentIndex === 0) ? action.currentIndex : state.length , 0, action.template ), [...state])) ||

  ...
  handleUpdateAnswer = (answerIndex: number, newAnswer: string) =>
    this.setState(prevState => ({
      ...
      answers: prevState.answers.map((ans: any, index) => {
        const temp = index === answerIndex ? newAnswer : ans;
        return temp;
      }),
    }));
      ...
      <div>
        ...
        <div className="clear-fix multiple-answer">
              ...
              <div className="radio-answer" key={answerIndex}>
                ...
                <div className="input-field input-text-radio input-option-create">
                  <TextField
                    ...
                    fullWidth
                    value={answer}
                    onChange={(e: any) => handleUpdateAnswer(answerIndex, e.target.value)}
                  />
                </div>
              </div>
            );
          ...
          <div className="radio-answer align-center">
            <RaisedButton label="More option" primary={true} onClick={e => handleAddAnswer()} />
          </div>
        </div>
      </div>
    );

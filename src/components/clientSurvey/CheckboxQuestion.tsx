...
> {
  ...
  updateAnswer = (indexAnswer: number) => {
    this.setState(prevState => ({
      ...
      answers: prevState.answers.map((answer, index) => {
        index === indexAnswer ? (answer.chosen = !answer.chosen) : "";
        return answer;
      }),
    }));
  };
  ...
  renderClientForm = () => {
    const { props: { questionIndex, removeQuestion, questionData } } = this;
  ...
  render() {
    return <div className="question-component">{this.renderClientForm()}</div>;
  }
